...
@Component({
  ...
  styleUrls: ['./flyer-detail.component.css'],
})
export class FlyerDetailComponent implements OnInit {
  ...
  loadDrops() {
    ...
    var flyerContent = this.flyerContent;
    $(() => {
      flyerContent.forEach((item: GenElement) => {
      ...
      (<any>$('#image-section')).droppable({
        drop: (event: any, ui: any) => {
          ...
          {
            var attributeIndex = this.flyerContent.findIndex(
              (x) => x.id == attributeDetail.Id
            );
            var attribute = this.flyerContent[attributeIndex];
            attribute.top = attributeDetail.PositionY;
            attribute.left = attributeDetail.PositionX;
            this.flyerContent[attributeIndex] = attribute;
