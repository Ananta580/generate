...
export class PostDetailComponent implements OnInit {
  ...
  loadDrops() {
    ...
    var postContent = this.postContent;
    $(() => {
      postContent.forEach((item: GenElement) => {
      ...
      (<any>$('#image-section')).droppable({
        drop: (event: any, ui: any) => {
          ...
          {
            var attributeIndex = this.postContent.findIndex(
              (x) => x.id == attributeDetail.Id
            );
            var attribute = this.postContent[attributeIndex];
            attribute.top = attributeDetail.PositionY;
            attribute.left = attributeDetail.PositionX;
            this.postContent[attributeIndex] = attribute;
